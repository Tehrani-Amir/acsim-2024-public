...
class MavDynamics:
    def __init__(self, Ts):
        ...
        # defining the 12 state variables (but in Quaternion is 13 variables) as a following vector:
        #zeros means the initial conditions
    ...
    def _f(self, state, forces_moments):
        ...
        east_dot = u*2*(e1*e2 + e3*e0) + v*2*(e2**2 + e0**2 - e1**2 - e3**2) + w*2*(e2*e3 - e0*e1)
        down_dot = u*2*(e1*e3 - e0*e2) + v*2*(e2*e3 + e0*e1) + w*(e3**2 + e0**2 - e1**2 - e2**2)
